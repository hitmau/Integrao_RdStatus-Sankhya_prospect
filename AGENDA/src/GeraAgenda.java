...
public class GeraAgenda implements AcaoRotinaJava {
	...
	public void doAction(ContextoAcao contexto) throws Exception {
		...
		query.nativeSelect("SELECT "+
				  ...
				  ", AD.OPORTUNIDADE   "+
				  ", AD.AGENDAMKT   "+
				  ", (SELECT PAP.NOMEPAP FROM TCSPAP PAP WHERE AD.CODPAP=PAP.CODPAP) AS NOMEPROSPECT   "+
				  ", (SELECT CTT.NOMECONTATO FROM TCSCTT CTT WHERE CTT.CODPAP = AD.CODPAP AND CTT.CODCONTATO = AD.IDCTT) AS NOMECONTATO   "+
				  ", AD.CTTNDECISOR AS CTTND   "+
				  ", (SELECT CTT.EMAIL FROM TCSCTT CTT WHERE AD.CODPAP=CTT.CODPAP AND CTT.CODCONTATO = AD.IDCTT) AS EMAIL   "+
				 ...
				 " WHERE NVL(AD.ENVIO, 'N') = 'N'  "+
				     " AND AD.CODUSU IN (SELECT DISTINCT CASE WHEN (SELECT U.CODGRUPO FROM TSIUSU U WHERE U.CODUSU IN   /*this.pUser*/ 26 ) IN (12,13, 1)   "+
				       " THEN (SELECT distinct U.CODUSU FROM TSIUSU U WHERE U.CODUSU=USU.CODUSU)   "+
				       " ELSE (SELECT U.CODUSU FROM TSIUSU U WHERE U.CODUSU =   /*this.pUser*/ 26  ) END   "+
				       " FROM TSIUSU USU)  "+
				     " AND NVL(AD.REMOVER, 'N') <> 'S' "+
				    " AND AD.TABELA = 'tcspap'");
		//"SELECT AD.CODPAP, AD.CTTNDECISOR, (SELECT CTT.EMAIL FROM TCSCTT CTT WHERE AD.CODPAP=CTT.CODPAP AND AD.IDCTT = CTT.CODCONTATO) AS EMAIL, (SELECT ID.DESCRICAO FROM AD_IDENTIFICADORESRD ID WHERE ID.IDMASC = AD.IDMASC) AS IDENDIFICADOR FROM AD_CONVMKTDIGITAL AD WHERE NVL(AD.ENVIO, 'N') = 'N'");
		while(query.next())
		{
			...
			GeraAgenda http = new GeraAgenda();
			...
			if (this.pAgendaMKT.equals("I")) { 
				...
				for (int i = 1 ; i < 4 ; i++){
					...
					Registro mkt = contexto.novaLinha("TGFTEL");
						mkt.setCampo("CODPARC", 1);
						mkt.setCampo("DHCHAMADA", getDateTime() + ":" + pSegundos); //getDateTime()
						mkt.setCampo("COMENTARIOS", "Prospect: " + this.pCodpap + " - " +this.pNomePap+ " | Contato: " + this.pNomeCtt + " | Follow Up " + i + " - Fluxo automático." );
						...
						mkt.setCampo("AD_CODCONTATO", this.pIdCtt);
